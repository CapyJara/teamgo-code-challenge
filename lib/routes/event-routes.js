...
router
  .post('/', ensureAuth(), async(req, res, next) => {
    ...
    if(!name || !description || !eventDate || !state || !city || !address) {
      return returnErr('Name, Description, state, city, address and EventDate required', next);
    }
  ...
  .get('/', async(req, res, next) => {
    ...
    if(city && !state) return returnErr('If searching by city, include state', next);
    if(start && !end || end && !start) return returnErr('If searching by date, include start and end dates', next);    
    
    // eslint-disable-next-line no-useless-escape
    const regex = RegExp(/\d{4}-\d{2}-\d{2}/);
    if(start && (!regex.test(start) || !regex.test(end))) return returnErr('Enter dates in yyyy-mm-dd format', next);
  ...
  .get('/:id', async(req, res, next) => {
    ...
    catch(e) { return returnErr('No event matching id', next); }
  ...
  .put('/update/:eventId', ensureAuth(), async(req, res, next) => {
    ...
    if(String(event.organizer) !== req.user.id) return returnErr('You do not have permission do update this event', next);
    return res.send(event);
  })
;
