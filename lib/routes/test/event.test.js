...
describe('event route tests', () => {
  ...
  it('organizer can update their own event', async() => {
    ...
    return await request(app)
      ...
      .set({ Authorization: user.token })
      .then(({ body }) => {
        expect(body.state).toEqual('NY');
        expect(body.city).toEqual('QUEENSBURY');
        expect(body.address).toEqual('21 LAKEVIEW DR');
      });
  });
  ...
  it('search with only city', async() => {
    return request(app)
      .get('/api/v1/event')
      .query({
        city: 'portland'
      ...
      .then(({ body }) => {
        console.log(body);
        expect(body).toEqual({
          message: 'If searching by city, include state',
          status: 500
        });
      });
  });

  it('search with only start date', async() => {
    return request(app)
      .get('/api/v1/event')
      .query({
        start: '2020-01-02'
      ...
      .then(({ body }) => {
        expect(body).toEqual({
          message: 'If searching by date, include start and end dates',
          status: 500
        });
      });
  });

  it('search with bad dates', async() => {
    return request(app)
      .get('/api/v1/event')
      .query({
        start: '2020-01-02',
      ...
      .then(({ body }) => {
        expect(body).toEqual({
          message: 'Enter dates in yyyy-mm-dd format',
          status: 500
        });
      });
  });
